    ...
    while default_cells[select_index] == "_":
        ...
        for select in default_cells:
            if select == "X":
                ...
                num_ += 1
        if abs(x_num - o_num) > 1 or o_num == 0:
          cordinate_val = input("Enter the coordinates: ")
          looping(cordinate_val)
        check_winner(default_cells)"""
    else:
...
def new_o_cell_value(x, y):
    ...
    while default_cells[select_index] == "_":
        ...
        n_cordinate_val = input("Enter the coordinates: ")
        looping(n_cordinate_val)
        # while check_cell(n_cordinate_val.replace(" ", "")):
        #     n_row, n_col = n_cordinate_val.split()
        #     while check_digits(n_row) and check_digits(n_col):
        #         new_o_cell_value(n_row, n_col)
        #     else:
...
def looping2(x, y):
    ...
    while check_digits(x) and check_digits(y):
        #while new_cell_value(x, y):
            #row1, col2 = input("Enter the coordinates: ").split()
